...
class Scheduler:
    ...
    class SchedulerCSP(csp.CSP):
        ...
        def nconflicts(self, course, term, assignment):
            ...
            return count(constraint(course, term, assignment) for constraint in self.constraints[course])
    ...
    def elective_constraint(self, **kwargs):
        ...
        term = kwargs['term']
        assignment = kwargs['assignment']
        assigned_electives = [elective for elective in self.electives if
                              (elective in assignment.keys() and assignment[elective] is not None)]
        if term is None and course in assigned_electives:
            ...
            assigned_electives.append(course)
        max_electives = self.program_requirements.at[self._get_program(), 'elective_courses_required']
    ...
    def _create_csp(self):
        ...
        transfers_and_taken = self.student_list.at[self.current_student, self.TRANSFERS] + self.student_list.at[
            self.current_student, self.TAKEN]
        variables = []
        ...
        while len(available_courses) > 0:
            ...
            for course in available_courses:
                if course in self.electives:
                    domains[course] = [None]
                for prerequisite in set(self.course_list.at[course, self.PREREQUISITES]).difference(
                ...
                domains[course] += list(range(lower_bound,
                                              max_terms + 1))  # for i in range(lower_bound,max_terms+1):  #     domains[course].append(i)
            lower_bound += 1
            if lower_bound > max_terms:
                return None
            available_courses = list(self._generate_available_courses(variables + transfers_and_taken).index)
        return self.SchedulerCSP(variables, domains, constraints)
